...
const InfluencerDetails: React.FC<Props> = ({
  influencer,
  products,
  backBtn = true,
  isModal = false,
  ...
  const getInfluencerProducts = async() => {
      const response = await getDocs(collection(db,'Influencers',influencer?.id,'Products'))
       let data = []
  ...
  const { price, basePrice, discount } = usePrice({
    amount: Number(influencer?.discountedPrice),
    baseAmount: Number(influencer?.totalPrice)
  });
  ...
  const variations = useMemo(
    () => getVariations(influencer?.variations),
    [influencer?.variations]
  );
  const isSelected = isVariationSelected(variations, attributes);
  let selectedVariation: any = {};
  if (isSelected) {
    selectedVariation = influencer?.variation_options?.find((o: any) =>
      isEqual(
