...
import CustomInfluencerDetailsModal from '@/components/ui/modal/customInfluencerDetailsModal';
type HeliumProps = {
  influencer: any;
  className?: string;
};

const InfluencerHelium: React.FC<HeliumProps> = ({ influencer, className }) => {
  ...
  const { price, basePrice,  } = usePrice({
    amount: Number(influencer?.totalPrice),
    baseAmount: influencer?.savedPrice,
  });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Image
          src={influencer?.imageUrl ?? productPlaceholder}
          alt={title}
          layout="fill"
          objectFit="contain"
      ...
      <CustomInfluencerDetailsModal  onClose={()=>{setOpen(false)}} open={open} influencer={influencer}/>
    </article>
  );
};

export default InfluencerHelium;
